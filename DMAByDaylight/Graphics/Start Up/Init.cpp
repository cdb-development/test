...
std::shared_ptr<CheatFunction> Cache = std::make_shared<CheatFunction>(2000, [] {  // Reduced to 2 seconds for faster recovery
    if (!EngineInstance) {
    ...
    if (base == 0) {
        ...
        // Try re-initializing the process
    ...
    // Check GWorld validity (using your offset)
